...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        z = 1.96  # 95% CI
        lower_bounds = preds - z * sigma_hat
        upper_bounds = preds + z * sigma_hat

        # 保存置信区间
        np.save(folder_path + 'pred_lower.npy', lower_bounds)
        np.save(folder_path + 'pred_upper.npy', upper_bounds)
        ...
        for i in range(0, len(preds), 20):
            ...
            ci_l = np.concatenate((input[:, -1], lower[:, -1]), axis=0)
            ci_u = np.concatenate((input[:, -1], upper[:, -1]), axis=0)

            visual(gt, pd, os.path.join(folder_path, str(i) + '.pdf'), lower=ci_l, upper=ci_u)
        # result save
        folder_path = './results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
