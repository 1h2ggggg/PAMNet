...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, PAMNet
...
class Exp_Main(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if 'Linear' in self.args.model or 'TST' in self.args.model or 'Net' in self.args.model or 'P_' in self.args.model:
    ...
    def train(self, setting):
        ...
        # heatmap_dir1 = "./PEA/"
        # if not os.path.exists(heatmap_dir1):
        #     os.makedirs(heatmap_dir1)
        time_now = time.time()
        epoch_time_sum = 0
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # print(f"Epoch {epoch + 1}, Iter {i}, adp max: {adp.max()}, min: {adp.min()}")
                # adp = x_pea.detach().cpu().numpy()
                # plt.figure()
                # sns.heatmap(adp[0, :, :], cmap="plasma", annot=False)
                # # plt.title(f"Epoch {epoch + 1}, Iter {i}", fontsize=20)
                # plt.savefig(os.path.join(heatmap_dir, f"iter{i}.pdf"))
                # plt.show()
                f_dim = -1 if self.args.features == 'MS' else 0
                # print(outputs.shape,batch_y.shape)
                outputs = outputs[:, -self.args.pred_len:, f_dim:]
